...
struct HealthKitRunsCacheSnapshot: Codable {
...
final class HealthKitRunsCache: HealthKitRunsCaching, @unchecked Sendable {
    static let shared = HealthKitRunsCache()

    private let queue = DispatchQueue(label: "com.athly.healthkitrunscache", qos: .utility)
    private let fileURL: URL
    private var cached: HealthKitRunsCacheSnapshot?
    ...
    private init() {
        ...
        self.fileURL = dir.appendingPathComponent("healthkit-runs-cache.json")
        loadFromDisk()
    }
    ...
    private func loadFromDisk() {
        ...
        let decoder = JSONDecoder()
        decoder.dateDecodingStrategy = .iso8601
        cached = try? decoder.decode(HealthKitRunsCacheSnapshot.self, from: data)
    }
