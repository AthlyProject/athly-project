...
export async function handleApiError<T>(
  ...
  // Check for missing permission scope (e.g. activity:read_permission code: "missing")
  // This means the token lacks the required OAuth scope — refreshing won't help.
  if (axios.isAxiosError(error) && error.response?.status === 401) {
    const responseData = error.response?.data as any;
    const errors = responseData?.errors;
    if (Array.isArray(errors) && errors.some((e: any) => e.code === 'missing')) {
      throw new Error(
        `NO_RUNNING_DATA: ${responseData?.message ?? 'Missing activity read permission'}`,
      );
    }
  }
  ...
  // Rate limit (429): do not retry; throw clear message
  if (axios.isAxiosError(error) && error.response?.status === 429) {
    const msg = (error.response?.data as { message?: string })?.message ?? 'Too Many Requests';
    console.error(`Rate limit hit in ${context}. Wait 15+ minutes.`);
    throw new Error(
      `429 Rate limit exceeded: ${msg}. Strava allows 100 requests per 15 minutes, 1000 per day.`,
    );
  }
...
export async function getRecentActivities(accessToken: string, perPage = 30): Promise<any[]> {
  ...
  try {
    const data = await cachedStravaGet<any[]>(accessToken, 'athlete/activities', {
      per_page: perPage,
    });
    ...
    // No refresh/retry — surface the error directly so the caller can handle it
    return await handleApiError<any[]>(error, 'getRecentActivities');
  }
}
...
export async function getAllActivities(
  ...
  try {
    while (hasMore) {
      ...
      if (maxPages !== undefined && currentPage - page >= maxPages) break;

      // Delay between pages to reduce rate-limit pressure (Strava: 100 req/15min, 1000/day)
      if (hasMore) {
...
export interface GetAllActivitiesParams {
  ...
  /** When set, stop after this many pages (avoids burning rate limit; use 1 for single-page fetch). */
  maxPages?: number;
}
...
export async function getActivityPhotos(
  ...
  try {
    const data = await cachedStravaGet<unknown>(
      accessToken,
