...
const GetAllActivitiesInputSchema = z.object({
  startDate: z
    .string()
    .optional()
    .describe("ISO date string for activities after this date (e.g., '2024-01-01')"),
  endDate: z
    .string()
    .optional()
    .describe("ISO date string for activities before this date (e.g., '2024-12-31')"),
  activityTypes: z
    .array(z.string())
    .optional()
  ...
  maxActivities: z
    ...
    .optional()
    .default(200)
    .describe('Maximum activities to return after filtering (default: 200)'),
  maxApiCalls: z
    .number()
    .int()
    .positive()
    ...
    .default(5)
    .describe(
      'Max API calls to avoid rate limit (Strava: 100/15min). Default 5 = up to ~1000 activities.',
    ),
  perPage: z
    .number()
    .int()
  ...
  execute: async (input: GetAllActivitiesInput) => {
    ...
    try {
      ...
      while (hasMore && apiCalls < maxApiCalls && filteredActivities.length < maxActivities) {
        ...
        // Fetch a single page (maxPages: 1) so we respect maxApiCalls and avoid rate limit
        ...
        if (activityTypes && activityTypes.length > 0) {
          toFilter = toFilter.filter((a) =>
            activityTypes.some((type) => a.type?.toLowerCase() === type.toLowerCase()),
          );
        }
        ...
        if (sportTypes && sportTypes.length > 0) {
          toFilter = toFilter.filter((a) =>
            sportTypes.some((type) => a.sport_type?.toLowerCase() === type.toLowerCase()),
          );
        }
