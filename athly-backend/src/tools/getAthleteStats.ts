import { z } from 'zod';
import { getAthleteStats as fetchAthleteStats, StravaStats } from '../stravaClient.js';
...
function formatStats(stats: StravaStats): string {
  ...
  if (stats.biggest_climb_elevation_gain != null) {
    response +=
      format('Biggest Climb Elevation Gain', stats.biggest_climb_elevation_gain, 'm') + '\n';
  }
  ...
  for (const { key, label } of activityTypes) {
    ...
    for (const period of periods) {
      ...
      response +=
        format('Distance', totals.distance, 'km', totals.count, totals.moving_time) + '\n';
