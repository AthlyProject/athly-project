...
function formatSegmentEffort(effort: StravaDetailedSegmentEffort): string {
  ...
  if (effort.average_cadence !== undefined && effort.average_cadence !== null)
    details += `   - Avg Cadence: ${effort.average_cadence.toFixed(1)}\n`;
  if (effort.average_watts !== undefined && effort.average_watts !== null)
    details += `   - Avg Watts: ${effort.average_watts.toFixed(1)}\n`;
  if (effort.average_heartrate !== undefined && effort.average_heartrate !== null)
    details += `   - Avg Heart Rate: ${effort.average_heartrate.toFixed(1)} bpm\n`;
  if (effort.max_heartrate !== undefined && effort.max_heartrate !== null)
    details += `   - Max Heart Rate: ${effort.max_heartrate.toFixed(0)} bpm\n`;
...
export const getSegmentEffortTool = {
  ...
  execute: async ({ effortId }: GetSegmentEffortInput) => {
    ...
    if (!token) {
      ...
      return {
        content: [
          { type: 'text' as const, text: 'Configuration error: Missing Strava access token.' },
        ],
        isError: true,
      };
    }
