...
function formatActivityDetails(activity: StravaDetailedActivity): string {
  ...
  if (activity.average_cadence !== undefined && activity.average_cadence !== null)
    details += `   - Avg Cadence: ${activity.average_cadence.toFixed(1)}\n`;
  ...
  if (activity.perceived_exertion)
    details += `   - perceived exertion: ${activity.perceived_exertion}\n`;
  if (activity.suffer_score)
    details += `   - Relative effort (Training Impulse): ${activity.suffer_score}\n`;
  return details;
}
