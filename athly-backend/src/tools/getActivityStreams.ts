  ...
  if (stream.type !== 'latlng' && stream.type !== 'moving') {
    ...
    for (let i = 1; i < maxPoints - 1; i++) {
      ...
      if (
        ...
        maxVal !== undefined &&
        maxIdx !== targetIdx &&
        Math.abs(maxVal - targetVal) > targetVal * 0.1
      ) {
      ...
      if (
        ...
        minVal !== undefined &&
        minIdx !== targetIdx &&
        Math.abs(minVal - targetVal) > targetVal * 0.1
      ) {
...
export const getActivityStreamsTool = {
  ...
  execute: async ({
    id,
    types = ['time', 'distance', 'heartrate', 'cadence', 'watts'],
    resolution,
    series_type,
    page = 1,
    points_per_page = 100,
        ...
        switch (stream.type) {
          ...
          case 'velocity_smooth': {
            ...
            stats = {
              ...
              avg_kph:
                Math.round(
                  (velocityData.reduce((a, b) => a + b, 0) / velocityData.length) * 3.6 * 10,
                ) / 10,
            };
            break;
          }
        ...
        return {
          content: [
            // First message with metadata
            {
              type: 'text' as const,
              text:
                `📊 Activity Stream Data (${totalPoints} points)\n` +
                ...
                JSON.stringify(
                  ...
                  format === 'compact' ? undefined : null,
                  format === 'compact' ? undefined : 2,
                ),
            },
            // Data messages
            ...Array.from({ length: numChunks }, (_, i) => {
              ...
              return {
                type: 'text' as const,
                text:
                  `Message ${i + 2}/${numChunks + 1} (points ${chunkStart + 1}-${chunkEnd}):\n` +
                  JSON.stringify(
                    streamData,
                    format === 'compact' ? undefined : null,
                    format === 'compact' ? undefined : 2,
...
function calculateNormalizedPower(powerData: number[]): number {
  ...
  const avgPower = Math.pow(movingAvg.reduce((a, b) => a + b, 0) / movingAvg.length, 0.25);
