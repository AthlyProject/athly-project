...
const ExploreSegmentsInputSchema = z.object({
  bounds: z
    ...
    .regex(
      /^-?\d+(\.\d+)?,-?\d+(\.\d+)?,-?\d+(\.\d+)?,-?\d+(\.\d+)?$/,
      'Bounds must be in the format: south_west_lat,south_west_lng,north_east_lat,north_east_lng',
    )
    .describe(
      'The geographical area to search, specified as a comma-separated string: south_west_lat,south_west_lng,north_east_lat,north_east_lng',
    ),
  activityType: z
    .enum(['running', 'riding'])
    .optional()
...
export const exploreSegments = {
  ...
  execute: async ({ bounds, activityType, minCat, maxCat }: ExploreSegmentsInput) => {
    ...
    if ((minCat !== undefined || maxCat !== undefined) && activityType !== 'riding') {
      return {
        content: [
          {
            type: 'text' as const,
            text: "❌ Input Error: Climb category filters (minCat, maxCat) require activityType to be 'riding'.",
          },
        ],
        isError: true,
      };
    ...
    try {
      ...
      const response: StravaExplorerResponse = await fetchExploreSegments(
        token,
        bounds,
        activityType,
        minCat,
