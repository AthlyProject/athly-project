...
function generateZwoContent(segments: WorkoutSegment[]): string {
  const workoutSegments = segments
    .map((segment) => {
      const durationSeconds =
        segment.duration.unit === 'min' ? segment.duration.value * 60 : segment.duration.value;
...
export const formatWorkoutFile = {
  ...
  inputSchema: z.object({
    ...
    format: z
      .enum(['zwo'])
      .default('zwo')
      .describe("Output format (currently only 'zwo' is supported)"),
  }),
