...
function formatDistribution(
  buckets: { max: number; min: number; time: number }[] | undefined,
): string {
  ...
  execute: async () => {
    ...
    try {
      ...
      if (errorMessage.includes('403')) {
        userFriendlyMessage =
          "🔒 Access denied. This tool requires 'profile:read_all' permission. Please re-authorize with the correct scope.";
      } else if (errorMessage.startsWith('SUBSCRIPTION_REQUIRED:')) {
        // In case Strava changes this later
