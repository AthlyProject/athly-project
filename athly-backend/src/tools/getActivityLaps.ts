...
const inputSchema = z.object({
  id: z
    ...
    .describe('The identifier of the activity to fetch laps for.'),
});
      ...
      const lapSummaries = laps.map((lap) => {
        const details = [
          ...
          lap.total_elevation_gain
            ? `  Elevation Gain: ${lap.total_elevation_gain.toFixed(1)} m`
            : null,
          ...
          lap.average_watts
            ? `  Avg Power: ${lap.average_watts.toFixed(1)} W ${lap.device_watts ? '(Sensor)' : ''}`
            : null,
        ];
